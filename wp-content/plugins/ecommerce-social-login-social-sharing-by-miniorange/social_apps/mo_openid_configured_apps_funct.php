<?php
function mo_wc_openid_validate_code()
{
...
function mo_wc_get_permalink($appname)
{
...
function wc_get_social_app_redirect_uri($appname){
    $apps2_list = array("line","dribbble","discord","stack","flickr","apple","tumblr","spotify","meetup");
    $appsConfig=0;
    foreach($apps2_list as $key ){
        if($appname==$key){
...
function mo_wc_openid_get_access_token($postData,$access_token_uri,$appname){
    $headers = '';
...
function mo_wc_openid_get_social_app_data($access_token,$profile_url,$app_name){
    $headers='';
...
function mo_wc_openid_app_test_config($profile_json_output){
    update_option( 'mo_openid_test_configuration', 0);
    ...
    $print .= mo_wc_openid_json_to_htmltable($profile_json_output);
    echo $print;
    exit;
}

function mo_wc_openid_json_to_htmltable($arr) {
    ...
    foreach ($arr as $key => $val) {
        ...
        if (is_array($val)) {
            if (!empty($val)) {
                $str .= mo_wc_openid_json_to_htmltable($val);
            }
        } else {
...
function mo_wc_openid_get_wp_style(){
    $path = site_url();
    $path .= '/wp-admin/load-styles.php?c=1&amp;dir=ltr&amp;load%5B%5D=dashicons,buttons,forms,l10n,login&amp;';
    return $path;
}

function mo_wc_openid_insert_query($social_app_name,$user_email,$userid,$social_app_identifier){
...
function mo_wc_openid_start_session_login($session_values){
...
function mo_wc_openid_login_user($user_id,$user,$user_picture){
...
function mo_wc_openid_delete_account_linking_rows($user_id){
    global $wpdb;
    $db_prefix = $wpdb->prefix;
...
function mo_wc_openid_profile_completion_form($user_val,$existing_uname='1'){
