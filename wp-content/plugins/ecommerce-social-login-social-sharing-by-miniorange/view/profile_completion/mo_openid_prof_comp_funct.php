...
function mo_wc_openid_save_profile_completion_form($username, $user_email, $first_name, $last_name, $user_full_name, $user_url, $user_picture, $decrypted_app_name, $decrypted_user_id){
    if(!isset($_POST['otp_field'])) {
        ...
        //email doesnt exist, check if username is in db or not, acc show form and proceed further
            ...
            if( isset($username_user_id) ){
                ...
                echo mo_wc_openid_profile_completion_form($user_details,'0');
                exit;
            }
            else {
...
function mo_wc_openid_validate_otp_form($username, $user_email, $transaction_id, $user_picture, $user_url, $last_name, $user_full_name ,$first_name, $decrypted_app_name, $decrypted_user_id,$message = ''){
    $path = mo_wc_openid_get_wp_style();
    ...
    if(get_option('moopenid_logo_check_prof') == 1) {
        $html .= mo_wc_openid_customize_logo();
    }
...
function mo_wc_send_otp_token($email){
...
function mo_wc_validate_otp_token($transactionId,$otpToken){
    mo_wc_openid_start_session();
    $customerKey = get_option('mo_openid_admin_customer_key');
    if($_SESSION['mo_otptoken']){
        $pass =	mo_wc_checkTimeStamp($_SESSION['sent_on'],time());
        $pass = mo_wc_checkTransactionId($customerKey, $otpToken, $transactionId, $pass);
        if($pass)
...
function mo_wc_openid_social_login_validate_otp($username, $user_email, $first_name, $last_name, $user_full_name, $user_url, $user_picture, $decrypted_app_name, $decrypted_user_id, $otp_token, $transaction_id){

    $validate_content = mo_wc_validate_otp_token($transaction_id, $otp_token);
    $status = $validate_content['status'];
    //if invalid OTP
    if($status == 'FAILURE'){
        $message = 'You have entered an invalid verification code. Enter a valid code.';
        echo mo_wc_openid_validate_otp_form($username, $user_email, $transaction_id, $user_picture, $user_url,  $last_name, $user_full_name,$first_name, $decrypted_app_name, $decrypted_user_id,$message);
        exit;
    ...
    //if OTP is Valid
        ...
        if ((isset($email_user_id)) || (isset($existing_email_user_id))) {
            ...
            mo_wc_openid_start_session_login($user_details);
            mo_wc_openid_login_user($user_id,$user,$user_picture);
            exit;
        }else{
            mo_wc_create_new_user($user_details);
        }

    }
}

function mo_wc_openid_profile_comp_action(){
