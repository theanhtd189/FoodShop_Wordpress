...
function mo_wc_openid_sso_sort_action(){
...
function mo_wc_openid_sso_enable_app()
{
    ...
    if (!wp_verify_nonce($nonce, 'mo-openid-sso-enable-app')) {
        ...
        $wc_enable_app=sanitize_text_field($_POST['app_name']);
        if ($enable_app == "true") {
            update_option('mo_openid_' . $wc_enable_app . '_enable', 1);
        } else if ($enable_app == "false") {
            update_option('mo_openid_' . $wc_enable_app . '_enable', 0);
        }
    }
}

//to load instructions of custom app
function mo_wc_openid_app_instructions_action()
{
    ...
    if (!wp_verify_nonce($nonce, 'mo-openid-app-instructions')) {
        ...
        $instructions = mo_wc_plugin_url . $social_app . ".png##";
...
function mo_wc_openid_capp_details_action()
{
...
function mo_wc_openid_capp_delete(){
    $appname = stripslashes(sanitize_text_field($_POST['app_name']));
    $appslist = maybe_unserialize(get_option('mo_openid_apps_list'));
...
function mo_wc_disable_app(){
    $appname = sanitize_text_field($_POST['app_name']);
    update_option('mo_openid_enable_custom_app_'.$appname,0);
    update_option('mo_openid_'.$appname.'_enable',0);
}

function mo_wc_openid_test_configuration_update_action(){
    update_option('mo_openid_test_configuration',1);
}

function mo_wc_attribute_url(){
    $url = home_url();
    return $url;
}

function mo_wc_custom_app_enable_change_update()
{
...
function mo_wc_register_customer_toggle_update(){
    if(mo_wc_openid_is_customer_registered()){
        $appname = stripslashes(sanitize_text_field($_POST['appname']));
        if(isset($appname))
            update_option('mo_openid_enable_custom_app_' . $appname, 0);
...
function mo_wc_openid_check_capp_enable(){
    if (get_option('mo_openid_apps_list')){
        $appslist = maybe_unserialize(get_option('mo_openid_apps_list'));
        $appname = stripslashes(sanitize_text_field($_POST['app_name']));
        if(!empty($appslist[$appname]['clientid']) && !empty($appslist[$appname]['clientsecret'])){
