...
function mo_wc_count_convert($count)
{
    if($count>=1000000){
        $count=$count/1000000;
        $count=$count."M";
...
function mo_wc_openid_share_shortcode( $atts = '', $title = '', $excerpt = '' ) {
        ...
        if($custom_theme == 'custom'){
            ...
            if( get_option('mo_openid_facebook_share_enable') ) {
                ...
                if ($sharing_counts){
                    if(get_option('mo_openid_Facebook_share_count_api')!="") {
                        ...
                        if ($count===false) {
                            $content = wp_remote_get("https://graph.facebook.com/v3.3/?id=" . $url . "&fields=engagement&access_token=".get_option('mo_openid_Facebook_share_count_api'));
            ...
            if( get_option('mo_openid_vkontakte_share_enable') ) {
                ...
                if ($sharing_counts){
                    ...
                    if($count===false) {
                        $content = wp_remote_get("https://vk.com/share.php?act=count&url=".$url);
                        $content=explode(',',$content);
                        $content=explode(')',$content[1]);
                        $count=$content[0];
                        $count=mo_wc_count_convert($count);
            ...
            if( get_option('mo_openid_stumble_share_enable') ) {
                ...
                if ($sharing_counts){
                    ...
                    if($count===false) {
                        $content = wp_remote_get("http://www.stumbleupon.com/services/1.01/badge.getinfo?url=" . $url);
                        $content = explode(',', $content);
            ...
            if( get_option('mo_openid_pinterest_share_enable') ) {
                if ($sharing_counts){
                    ...
                    if($count==false) {
                        $content_pint = wp_remote_get("https://widgets.pinterest.com/v1/urls/count.json?source=6&url=".$url);
                        $body = preg_replace( '/^receiveCount\((.*)\)$/', '\\1', $content_pint );
                        $count=explode(":",$body);
                        $count=explode("}",$count[3]);
                        $count=mo_wc_count_convert($count[0]);
...
function mo_wc_openid_vertical_share_shortcode( $atts = '', $title = '', $excerpt = '' ) {
    wp_enqueue_style( 'mo-wp-style-icon',plugins_url('includes/css/mo_openid_login_icons.css?version=7.3.0', __FILE__), false );
    if( get_option('mo_openid_fonawesome_load') == 1)
    wp_enqueue_style( 'mo-openid-sl-wp-font-awesome',plugins_url('includes/css/mo-font-awesome.min.css', __FILE__), false );
    wp_enqueue_style( 'mo_openid_admin_settings_style', plugins_url('includes/css/mo_openid_style.css?version=7.3.0', __FILE__));
