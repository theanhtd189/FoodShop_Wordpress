...
/**
 * Plugin Name: Ecommerce Social Login, Social Sharing by miniOrange
...
define('ECOMMERCE_SOCIAL_LOGIN_SOCIAL_SHARING_BY_MINIORANGE', '1.1.6');
define('mo_wc_plugin_url', plugin_dir_url(__FILE__) . "includes/images/icons/");
define('MO_WC_MOSL_PLUGIN_DIR',str_replace('/','\\',plugin_dir_path(__FILE__)));
require('miniorange_wc_openid_sso_settings_page.php');
...
require('class-mo-wc-openid-sso-shortcode-buttons.php');
require('class-mo-wc-openid-social-comment.php');
require('view/email_settings/mo_openid_email_settings_functions.php');
include dirname( __FILE__ ) . '/mo_openid_Language.php';
include dirname(__FILE__) . '/mo_wc_openid_feedback_form.php';
include_once dirname(__FILE__) . '/class-mo-wc-wcOpenid-login-widget.php';

class mo_wc_openid_sso_settings
{
    function __construct()
    {
        register_activation_hook( __FILE__, array( $this, 'mo_wc_openid_activate' ) );
        register_deactivation_hook(__FILE__, array( $this, 'mo_wc_openid_deactivate'));
        add_action( 'admin_footer', array( $this,'mo_wc_openid_feedback_request' ));
        add_action( 'plugins_loaded', array( $this, 'mo_wc_social_load_textdomain'));
        add_action('admin_menu', array($this, 'mo_wc_new_miniorange_openid_menu'));
        add_action( 'admin_enqueue_scripts', array( $this, 'mo_wc_openid_plugin_settings_admin_style' ) );


        add_action( 'admin_init',  array( $this, 'mo_wc_miniorange_openid_save_settings' ) );
        add_action('init', 'mo_wc_openid_login_validate' );
        add_action( 'plugins_loaded', 'mo_wc_openid_plugin_update' ,1 );
        add_action( 'admin_enqueue_scripts', array( $this, 'mo_wc_openid_plugin_settings_script' ) );


        add_action('wp_ajax_mo-openid-sso-sort-action', 'mo_wc_openid_sso_sort_action');
        add_action('wp_ajax_mo_openid_share', 'mo_wc_openid_share_action');
        add_action('wp_ajax_mo_openid_app_enable', 'mo_wc_openid_sso_enable_app');
        add_action('wp_ajax_mo_openid_app_instructions', 'mo_wc_openid_app_instructions_action');
        add_action('wp_ajax_mo_openid_capp_details', 'mo_wc_openid_capp_details_action');
        add_action('wp_ajax_mo_openid_capp_delete', 'mo_wc_openid_capp_delete');
        add_action('wp_ajax_mo_openid_test_configuration_update', 'mo_wc_openid_test_configuration_update_action');
        add_action('wp_ajax_mo_openid_profile_comp', 'mo_wc_openid_profile_comp_action');
        add_action('wp_ajax_custom_app_enable_change_update', 'mo_wc_custom_app_enable_change_update');
        add_action('wp_ajax_mo_register_customer_toggle_update', 'mo_wc_register_customer_toggle_update');
        add_action('wp_ajax_mo_openid_check_capp_enable', 'mo_wc_openid_check_capp_enable');
        add_action('wp_ajax_mo_register_new_user', 'mo_wc_openid_register_user');
        add_action('wp_ajax_mo_register_old_user', 'mo_wc_register_old_user');
        add_action('wp_ajax_mo_sharing_app_value', 'mo_wc_sharing_app_value');
        add_action('wp_ajax_mo_openid_rating_given', 'mo_wc_openid_rating_given');
        add_action('wp_ajax_mo_disable_app', 'mo_wc_disable_app');
        add_action( 'wp_enqueue_scripts', array( $this, 'mo_wc_openid_plugin_script' ) ,5);
        add_option('mo_wc_openid_rateus_activate','0');
        add_option('mo_wc_check_notice','1');
        //for addon
        add_action('wp_ajax_verify_addon_licience', 'mo_wc_openid_show_verify_addon_license_page');
        ...
        //add shortcode
        add_shortcode( 'miniorange_social_login', array($this, 'mo_wc_get_output') );
        add_shortcode( 'miniorange_social_sharing', array($this, 'mo_wc_get_sharing_output') );
        add_shortcode( 'miniorange_social_sharing_vertical', array($this, 'mo_wc_get_vertical_sharing_output') );
        add_shortcode( 'miniorange_social_custom_fields', array($this, 'mo_wc_get_custom_output') );
        add_filter( 'the_content', array( $this, 'mo_wc_openid_add_social_share_links' ) );
        add_filter( 'the_excerpt', array( $this, 'mo_wc_openid_add_social_share_links' ) );
        add_shortcode( 'miniorange_social_comments', array($this, 'mo_wc_get_comments_output') );
...
//
        ...
        update_option('app_pos_premium','apple#paypal#wordpress#disqus#github#hubspot#mailru#pinterest#yandex#spotify#reddit#tumblr#twitch#vimeo#kakao#discord#dribbble#flickr#line#meetup#stackexchange#livejournal#snapchat#foursquare#teamsnap#naver#odnoklassniki#wiebo#wechat#baidu#renren#qq');
        ...
        add_option( 'mo_openid_login_button_customize_text', mo_wc_sl('Login with' ));
        ...
        add_option( 'mo_openid_woocommerce_register_form_start','1');



        if(get_option('mo_openid_woocommerce_register_form_start') == 1){
            add_action( 'woocommerce_register_form_start', array($this, 'mo_wc_openid_add_social_login'));
        }

        if(get_option('mo_openid_woocommerce_before_login_form') == 1){
            add_action( 'woocommerce_login_form_start', array($this, 'mo_wc_openid_add_social_login'));
        }
        if(get_option('mo_openid_default_comment_enable') == 1 ){
            add_action('comment_form_must_log_in_after', array($this, 'mo_wc_openid_add_social_login'));
            add_action('comment_form_top', array($this, 'mo_wc_openid_add_social_login'));
        }
        if(get_option('mo_openid_social_comment_fb') == 1 || get_option('mo_openid_social_comment_disqus') == 1 ){
            add_action('comment_form_top', array( $this, 'mo_wc_openid_add_comment'));
        }

        // add social login icons to default login form
        if(get_option('mo_openid_default_login_enable') == 1) {
            add_action('login_form', array($this, 'mo_wc_openid_add_social_login'));
        }

        // add social login icons to default registration form
        if(get_option('mo_openid_default_register_enable') == 1){
            add_action( 'register_form', array($this, 'mo_wc_openid_add_social_login') );
        }
        ...
        if(get_option('moopenid_social_login_avatar')) {
            add_filter( 'get_avatar', array( $this, 'mo_wc_social_login_custom_avatar' ), 15, 5 );
            add_filter( 'get_avatar_url', array( $this, 'mo_wc_social_login_custom_avatar_url' ), 15, 3);
        }
...
Thank you.';
    ...
    function mo_wc_openid_plugin_settings_script() {
        if(strpos(get_current_screen()->id, 'ecommerce-social-login_page') === false) return;
    ...
    function mo_wc_openid_plugin_settings_admin_style(){
        if(strpos(get_current_screen()->id, 'ecommerce-social-login_page') === false) return;
    ...
    function mo_wc_openid_activate() {
    ...
    function mo_wc_new_miniorange_openid_menu() {
        //Add miniOrange plugin to the menu
        $page = add_menu_page( 'MO OpenID Settings ' . __( 'Configure OpenID', 'mo_wc_openid_settings' ), 'Ecommerce Social Login', 'administrator',
            'mo_wc_openid_settings', 'mo_wc_register_openid' ,plugin_dir_url(__FILE__) . 'includes/images/miniorange_icon.png');
        $page = add_submenu_page( 'mo_wc_openid_settings', 'MiniOrange-General Settings','Social Login', 'administrator','mo_wc_openid_general_settings','mo_wc_register_openid' );
        $page = add_submenu_page( 'mo_wc_openid_settings', 'MiniOrange-General Settings','Social Sharing', 'administrator','mo_wc_openid_social_sharing_settings','mo_wc_register_sharing_openid' );
        $page = add_submenu_page( 'mo_wc_openid_settings', 'MiniOrange-General Settings','Social Commenting', 'administrator','mo_wc_openid_social_commenting_settings','mo_wc_comment_openid');
        $page = add_submenu_page( 'mo_wc_openid_settings', 'MiniOrange-Add_On','Add-On', 'administrator','mo_wc_openid_settings_addOn','mo_wc_openid_addon_desc_page');
        $page = add_submenu_page( 'mo_wc_openid_settings', 'MiniOrange-WooCommerce_Add_On','WooCommerce Add-On', 'administrator','mo_openid_woocommerce_add_on','mo_wc_openid_addon_desc_page');
        $page = add_submenu_page( 'mo_wc_openid_settings', 'MiniOrange-BuddyPress_Add_On','BuddyPress Add-On', 'administrator','mo_openid_buddypress_add_on','mo_wc_openid_addon_desc_page');
        $page = add_submenu_page( 'mo_wc_openid_settings', 'MiniOrange-MailChimp_Add_On','MailChimp Add-On', 'administrator','mo_openid_mailchimp_add_on','mo_wc_openid_addon_desc_page');
        $page = add_submenu_page( 'mo_wc_openid_settings', 'MiniOrange-HubSpot_Add_On','HubSpot Add-On', 'administrator','mo_openid_hubspot_add_on','mo_wc_openid_addon_desc_page');

        remove_submenu_page('mo_wc_openid_settings','mo_wc_openid_settings');
    }


    function mo_wc_openid_add_social_login(){
        if(!is_user_logged_in() && strpos( $_SERVER['QUERY_STRING'], 'disable-social-login' ) == false){
            $mo_login_widget = new mo_wc_openid_login_wid();
            $mo_login_widget->mo_wc_openidloginForm();
        }
    }

    function mo_wc_openid_add_comment(){
        global $post;
        ...
        if(is_single() && get_option('mo_openid_social_comment_blogpost') == 1 ) {
            mo_wc_openid_social_comment($post, $url);
        } else if(is_page() && get_option('mo_openid_social_comment_static')==1) {
            mo_wc_openid_social_comment($post, $url);
        }
    }

    function mo_wc_openid_plugin_script() {
    ...
    function mo_wc_miniorange_openid_save_settings()
    {
        if(is_admin() && get_option('Activated_Plugin')=='Plugin-Slug') {
            delete_option('Activated_Plugin');
            update_option('mo_openid_message','Go to plugin <b><a href="admin.php?page=mo_wc_openid_settings">settings</a></b> to enable Social Login, Social Sharing by miniOrange.');
            add_action('admin_notices', 'mo_wc_openid_activation_message');
        }
        ...
        switch( $value )
        {
            ...
            case 'mo_openid_contact_us_query_option':
                ...
                if (!wp_verify_nonce($nonce, 'mo-openid-contact-us-nonce')) {
                    ...
                    if (mo_wc_openid_check_empty_or_null($email) || mo_wc_openid_check_empty_or_null($query)) {
                        ...
                        $submited = $customer->mo_wc_submit_contact_us($email, $phone, $query, $feature_plan);
                        if ($submited == false) {
            ...
            case 'mo_openid_rateus_query_option':
                $nonce = sanitize_text_field($_POST['mo_wc_openid_rateus_nonce']);
                //var_dump($nonce);exit();
                if (!wp_verify_nonce($nonce, 'mo-wc-openid-rateus-nonce')) {
                    ...
                    if (mo_wc_openid_check_empty_or_null($email) ||mo_wc_openid_check_empty_or_null($query)) {
                        ...
                        $submited = $customer->mo_wc_submit_rate_us($email, $query);
                        if ($submited == false) {
                            update_option('mo_openid_message', 'Your query could not be submitted. Please try again.');
                            mo_wc_openid_show_error_message();
                        } else {
            ...
            case 'mo_openid_enable_woocommerce_display':
                $nonce=sanitize_text_field($_POST['mo_openid_enable_woocommerce_display_nonce']);
                if (!wp_verify_nonce($nonce, 'mo-openid-enable-woocommerce-display-nonce')) {
                    wp_die('<strong>ERROR</strong>: Invalid Request.');
                }else {
            ...
            case 'mo_openid_verify_license':
                ...
                if (!wp_verify_nonce($nonce, 'mo-openid-verify-license-nonce')) {
                    ...
                    $customer = new MO_WC_CustomerOpenID();
                    $content = json_decode($customer->mo_wc_check_customer_ln($_POST['licience_type']), true);
                    if (strcasecmp($content['status'], 'SUCCESS') == 0) {
                        $content = json_decode($customer->mo_wc_openid_vl($code, false), true);
                        ...
                        if (strcasecmp($content['status'], 'SUCCESS') == 0) {
                            ...
                            if($_POST['licience_type']=="extra_attributes_addon") {
                                update_option('mo_openid_opn_lk_extra_attr_addon',  MO_WC_MOAESEncryption::mo_wc_encrypt_data($code, $key));
                                ...
                                update_option('mo_openid_opn_lk_wca_addon',  MO_WC_MOAESEncryption::mo_wc_encrypt_data($code, $key));
                                ...
                                update_option('mo_openid_opn_lk_bpp_addon', MO_WC_MOAESEncryption::mo_wc_encrypt_data($code, $key));
                                ...
                                update_option('mo_openid_opn_lk_mailc_addon',  MO_WC_MOAESEncryption::mo_wc_encrypt_data($code, $key));
                                ...
                                update_option('mo_openid_opn_lk_hub_addon',  MO_WC_MOAESEncryption::mo_wc_encrypt_data($code, $key));
                                update_option('mo_openid_message', 'Your HubSpot addon license is verified. You can now setup the addon plugin.');
                            }
                            $key = get_option('mo_openid_customer_token');
                            update_option('mo_openid_site_ck_l', MO_WC_MOAESEncryption::mo_wc_encrypt_data("true", $key));
                            update_option('mo_openid_t_site_status', MO_WC_MOAESEncryption::mo_wc_encrypt_data("false", $key));
                            mo_wc_openid_show_success_message();
                        ...
                        $key = get_option('mo_openid_customer_token');
                        update_option('mo_openid_site_ck_l', MO_WC_MOAESEncryption::mo_wc_encrypt_data("false", $key));
                        $url = add_query_arg(array('tab' => 'pricing'), $_SERVER['REQUEST_URI']);
                        update_option('mo_openid_message', 'You have not upgraded yet. <a href="' . $url . '">Click here</a> to upgrade to premium version.');
                        mo_wc_openid_show_error_message();
                    }
                    $content = json_decode($customer->mo_wc_check_customer_valid(), true);
                    if (strcasecmp($content['status'], 'SUCCESS') == 0) {
            ...
            case 'mo_openid_feedback':
                ...
                if ( ! wp_verify_nonce( $nonce, 'mo-openid-feedback-nonce' ) ) {
                    ...
                    if(isset($_POST['deactivate_plugin']) )
                    {
                        ...
                        //only reason
                        $phone='';
                        $contact_us = new MO_WC_CustomerOpenID();
                        $submited = json_decode( $contact_us->mo_wc_openid_send_email_alert( $email, $phone, $message ), true );
                        ...
                        deactivate_plugins( '/ecommerce-social-login-social-sharing-by-miniorange/miniorange_wc_openid_sso_settings.php' );
    ...
    public function mo_wc_get_output( $atts ){
            $miniorange_widget = new mo_wc_openid_login_wid();
            $html = $miniorange_widget->mo_wc_openidloginFormShortCode( $atts );
            return $html;
    }
    function mo_wc_openid_deactivate() {
    ...
    function mo_wc_openid_feedback_request(){
        if(get_option('mo_openid_deactivate_reason_form')=='0')
            mo_wc_openid_display_feedback_form();
    }
    function mo_wc_openid_add_social_share_links($content) {
        global $post;
        $post_content=$content;
        $title = str_replace('+', '%20', urlencode($post->post_title));

        if(is_front_page() && get_option('mo_share_options_enable_home_page')==1){
            $html_content = mo_wc_openid_share_shortcode('', $title);
    ...
    function mo_wc_social_login_custom_avatar( $avatar, $mixed, $size, $default, $alt = '' ) {
    ...
    function mo_wc_social_login_custom_avatar_url( $url, $id_or_email, $args = null ) {
    ...
    public function mo_wc_get_sharing_output( $atts ){
        $title = '';
        global $post;
        if(isset($post)) {
        ...
        $html = mo_wc_openid_share_shortcode( $atts, $title);
        return $html;
    }

    public function mo_wc_get_vertical_sharing_output( $atts )
    {
        $title = '';
        global $post;
        if (isset($post)) {
        ...
        $html = mo_wc_openid_vertical_share_shortcode($atts, $title);
        return $html;
    }
    public function mo_wc_get_comments_output( $atts ){
        $html = mo_wc_openid_comments_shortcode();
        return $html;
    }

    public function mo_wc_get_custom_output()
    {
        $title = '';
        global $post;
        if (isset($post)) {
    ...
    function mo_wc_social_load_textdomain()
    {

            load_plugin_textdomain( 'ecommerce-social-login-social-sharing-by-miniorange', "", dirname( plugin_basename(__FILE__) ) . '/lang/' );

    }
}


new mo_wc_openid_sso_settings();